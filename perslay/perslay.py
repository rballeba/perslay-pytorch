...
def rational_layer(inp, mu, sg, al):
    """ Rational PersLay """
    mu, sg, al = tf.expand_dims(tf.expand_dims(mu, 0), 0), tf.expand_dims(tf.expand_dims(sg, 0), 0), tf.expand_dims(
        tf.expand_dims(al, 0), 0)
    dimension_before, num_pts = inp.shape[2], inp.shape[1]
    bc_inp = tf.expand_dims(inp, -1)
    return 1 / tf.math.pow(1 + tf.math.reduce_sum(tf.math.multiply(tf.math.abs(bc_inp - mu), tf.math.abs(sg)), axis=2),
                           al)
...
def landscape_layer(inp, sp):
    ...
    return tf.math.maximum(
        .5 * (inp[:, :, 1:2] - inp[:, :, 0:1]) - tf.math.abs(sp - .5 * (inp[:, :, 1:2] + inp[:, :, 0:1])),
        np.array([0]))
...
def entropy_layer(inp, theta, sp):
    ...
    return tf.math.multiply(entropy_terms,
                            1. / (1. + tf.math.exp(-theta * (.5 * (Y - X) - tf.math.abs(sp - .5 * (Y + X))))))


def image_layer(inp, image_size, image_bnds, sg, limit_corrector=1e-6):
    """ Persistence Image PersLay """
    bp_inp = tf.einsum("ijk,kl->ijl", inp, tf.constant(np.array([[1., -1.], [0., 1.]], dtype=np.float32)))
    dimension_before, num_pts = inp.shape[2], inp.shape[1]
    coords = [tf.range(start=image_bnds[i][0], limit=image_bnds[i][1] - limit_corrector,
                       delta=(image_bnds[i][1] - image_bnds[i][0]) / image_size[i]) for i in range(dimension_before)]
    ...
    return tf.expand_dims(
        tf.math.exp(tf.math.reduce_sum(-tf.math.square(bc_inp - mu) / (2 * tf.math.square(sg)), axis=2)) / (
                    2 * np.pi * tf.math.square(sg)), -1)
    ...
    def compute_representations(self, diags, training=False):
        ...
        for nf, plp in enumerate(self.perslay_parameters):
            ...
            if plp["pweight"] == "power":
                ...
                weight = tf.expand_dims(tf.math.reduce_sum(tf.math.exp(
                    tf.math.reduce_sum(-tf.math.multiply(tf.math.square(bc_inp - M), tf.math.square(V)), axis=2)),
                                                           axis=2), -1)
